...
- With Intel compilers, this action sets up the oneAPI environment. Do not re-run environment setup scripts such as `setvars.bat` or `vcvarsall.bat` after this action runs. Doing so can cause the `PATH` variable to exceed character limits on Windows.
